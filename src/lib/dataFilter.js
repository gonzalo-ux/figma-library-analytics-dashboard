 ...
 * This applies both page filters (if enabled) and type-specific logic (e.g., icons filter)
...
export function filterDataForPage(data, config, pageId) {
  ...
  // Apply page exclusion filters if enabled
  if (page.useFilters && page.filters) {
    filtered = applyLibraryFilters(filtered, page.filters)
  }

  // Apply page-type specific filters
  switch (page.type) {
    case 'icons':
      // Only show items that look like icons
      filtered = filtered.filter(item => {
        const name = (item.component_name || '').trim()
        return name.toLowerCase().startsWith('icon -') || name.toLowerCase().includes('icon -')
      })
      break
    
    case 'components':
      // Exclude icons from components view (unless explicitly using filters)
      filtered = filtered.filter(item => {
        const name = (item.component_name || '').trim()
        return !name.toLowerCase().startsWith('icon -') && !name.toLowerCase().includes('icon -')
      })
    ...
    // variables and styles don't need additional filtering beyond page filters
    default:
      break
  }
